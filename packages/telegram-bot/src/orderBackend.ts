...
export function buildOrderPayload(args: {
  ...
  const meta: OrderPayload["meta"] = {}
  if (args.recipeTitle && args.recipeTitle.trim()) meta.recipe_title = args.recipeTitle.trim()
  if (typeof totalCents === "number") meta.total_cents = totalCents
...
export async function sendOrderToBackendRequest(args: SendOrderArgs): Promise<unknown> {
  ...
  const payloadToSend: OrderPayload = args.usageIdempotencyKey
    ? { ...args.orderPayload, usage_idempotency_key: args.usageIdempotencyKey }
    : args.orderPayload
    ...
    try {
      ...
      const response = await fetchImpl(endpoint, {
        ...
        headers: {
          ...
          "X-Valuya-Subject": args.subjectId,
      ...
      const err = new Error(`order_backend_http_${response.status}:${JSON.stringify(body).slice(0, 300)}`)
      ;(err as any).details = {
        status: response.status,
        body,
      ...
      if ((error as any)?.details?.status) throw error
...
function mapProducts(input: unknown): OrderProduct[] {
  ...
  for (const raw of input) {
    ...
    const product: OrderProduct = { sku, name, qty }
    if (typeof unitPrice === "number") product.unit_price_cents = unitPrice
    out.push(product)
  }
  return out
}
