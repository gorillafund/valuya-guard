...
function isFree(payment) {
    return payment.is_free === true || payment.method === "free";
}
function assertOkSubmit(res) {
...
export async function purchase(args) {
    ...
    if (!session.payment)
        throw new Error("session_missing_payment_instruction");
    if (!session.server_time)
        throw new Error("session_missing_server_time");
    ...
    const expires_at = new Date(serverMs + Number(session.agent_proof_ttl_seconds) * 1000).toISOString();
