import type { FileStateStore, StoredChannelLink } from "./stateStore.js"
...
export class GuardWhatsAppLinkService {
  ...
  constructor(args: {
    ...
    this.redeemedFrom = args.redeemedFrom?.trim() || "whatsapp_bot"
  ...
  private async post<T>(path: string, body: Record<string, unknown>): Promise<T> {
    ...
    console.log(
      JSON.stringify({
        level: "info",
        event: "guard_channel_request",
        path,
        channel_app_id: body.channel_app_id ?? this.channelAppId,
        channel_user_id: body.channel_user_id ?? null,
        redeemed_from: body.redeemed_from ?? null,
      }),
    )
...
export function extractLinkToken(text: string): string | null {
  ...
  const prefixed = /^link\b[\s:,-]*(.+)$/i.exec(value)
  const search = prefixed ? prefixed[1] : value
  const match = /\b(gls_[A-Za-z0-9._-]+)\b/i.exec(search)
  return match ? match[1] : null
}

export function normalizeWhatsAppUserId(from: string): string {
  ...
  if (!digits) throw new Error("twilio_channel_user_id_invalid")
  return digits
}
