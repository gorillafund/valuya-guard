export function buildAgentPaymentProofFromSession(args) {
    ...
    if (!session.required_hash)
        throw new Error("session_missing_required_hash");
    if (!session.pricing_hash)
        throw new Error("session_missing_pricing_hash");
    if (typeof session.quantity_effective !== "number")
        throw new Error("session_missing_quantity_effective");
    const payment = session.payment;
    if (!payment)
        throw new Error("session_missing_payment");
    ...
    if (!token_address)
        throw new Error("payment_missing_token_address");
    return {
